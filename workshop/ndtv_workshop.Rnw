...
% increase plot widths to full text width
...
\title{Workshop materials (outdated) for learning \Sexpr{foo$Package}: \Sexpr{foo$Title} (Package version \Sexpr{foo$Version})}  
...
This pdf version of the workship is somewhat outdated. The current version of the workshop is maintained as an HTML file  (webpage) at \url{http://statnet.csde.washington.edu/workshops/SUNBELT/current/ndtv/ndtv_workshop.html}
...
\subsection{Authors}
\begin{itemize}
  \item Developed by Skye Bender-deMoll (\url{skyebend@skyeome.net}) with members of the statnet \url{http://statnet.org} team. 
  \item This work was supported by grant R01HD68395 from the National Institute of Health.
\end{itemize}

\subsection{Workshop prerequisites}
\begin{itemize}
  \item Familiarity with the R statistical software.  We are not going to cover basics of how to use and install R.
  \item Functioning R installation and basic statnet packages already installed. (Instructions on installing R and statnet are located here \url{https://statnet.csde.washington.edu/trac/wiki/Installation}.)
  \item Familiarity with general network and SNA concepts
  \item Experience with statnet packages and \verb@network@ data structures preferred but not necessary. (A tutorial providing ``An Introduction to Network Analysis with R and statnet is located here \url{https://statnet.csde.washington.edu/trac/wiki/Resources}) 
  \item A working internet connection (we will install some libraries and download datasets)
\end{itemize}



\subsection{A Quick Demo}
...
library(ndtv)
@
Now we need some dynamic network data to explore. The package includes an example network data set named \verb@short.stergm.sim@ which is the output of a toy STERGM model based on Padgett's Florentine Family Business Ties dataset simulated using the \verb@tergm@ package\footnote{Using the tergm package to simulate the model is illustrated in a later example.}.  
...
Notice that the object has a class of both \verb@networkDynamic@ and \verb@network@.  All \verb@networkDynamic@ objects are still \verb@network@ objects, they just include additional special attributes to store time information.  
<<print_stergm>>=
print(short.stergm.sim)
@
The print command for \verb@networkDynamic@ objects includes some additional info about the time range of the network and then the normal output from \verb@print.network@.

\subsubsection{Render a network animation as a video}
...
\subsubsection{Plot a static ``filmstrip'' sequence}

An animation is not the only way to display a sequence of views of the network.  We could also use the \verb@filmstrip()@ function that will create a ``small multiple'' plot using frames of the animation to construct a static visual summary of the network changes.

<<filmstrip,fig=TRUE>>=
filmstrip(short.stergm.sim,displaylabels=FALSE)
@

\subsubsection{Plot a timeline}
...
\subsubsection{Plot a proximity timeline}
...
\subsubsection{Print tabular data}

Of course we can always display the edge (or vertex) spells directly in a tabular form using the various utilities in the \verb@networkDynamic@ package. 
...
\smallskip
\smallskip
\noindent\fbox{\parbox{\textwidth}{\textbf{Exercise:} Load the saved version of short.stergm.sim. Are there any edges that are present for the entire time period from 0 until 25?}}
\smallskip
...
\subsection{Installing ndtv and its external dependencies}
The \verb@ndtv@ \citep{ndtv} package relies on many other packages to do much of the heavy lifting, especially \verb@animation@ \citep{animation} and \verb@networkDynamic@. Web animations can be created and viewed on computers with modern web browsers without additional components.  However, \verb@ndtv@ does requires external libraries (FFmpeg) to save movies as video files, and Java to be able to some of the better layout algorithms. 

\subsubsection{Installing ndtv and its R package dependencies}
R can automatically install the packages \verb@ndtv@ depends on when \verb@ndtv@ is installed. So open up your R console, and run the following command:
<<installation>>==
...
\subsubsection{Installing FFmpeg for saving animations}
In order to save out animations as video files and use the better-quality layouts, we need to install some additional non-R software dependencies on the computer.  FFmpeg \url{http://ffmpg.org} is a cross-platform tool for converting and rendering video content in various formats. It is used as an external library by the \verb@animation@ package to save out the animation as a movie file on disk. (see \verb@?saveVideo@ for more information.) The install instructions are somewhat different on each platform.  You can also access these instructions using \verb@?install.ffmpeg@
<<install_ffmpeg>>==
?install.ffmpeg   # help page for installing ffmpeg
@
\begin{itemize}
\item \textbf{Windows instructions}
  \begin{itemize}
  ...
  \item Decompress the package and store contents on your computer (probably in \verb@Program Files@)
  
  \item Edit your system path variable to include the path to the directory containing \verb@ffmpeg.exe@
  \end{itemize}
\item \textbf{Mac instructions}
  \begin{itemize}
  \item Download most recent build from \url{http://www.evermeet.cx/ffmpeg/}
  ...
  \item Copy ffmpeg to \verb@/usr/local/bin/ffmpeg@
  \end{itemize}
\item \textbf{Linux/Unix instructions}
  \begin{itemize}
  \item FFmpeg is a standard package on many Linux systems. You can check if it is installed with a command like \verb@dpkg -s ffmpeg@. If it is not installed, you should be able to install with your system's package manager. i.e. \verb@sudo apt-get install ffmpeg@ or search 'ffmpeg' in the Software Center on Ubuntu.
  \end{itemize}
\end{itemize}

After you have installed FFmpeg on your system, you can verify that R knows where to find it by typing \verb@Sys.which('ffmpeg')@ in the R terminal. You many need to first restart R after the install.

\subsubsection{Installing Java and MDSJ setup}
To use the MDSJ layout algorithm, you must have Java installed on your system.  Java should be already installed by default on most Mac and Linux systems. If it is not installed, you can download it from \url{http://www.java.com/en/download/index.jsp}. On Windows, you may need to edit your ``Path'' environment variable to make Java executable from the command-line. Oracle provides some instructions for editing the Path: \url{http://www.java.com/en/download/help/path.xml}
...
Due to CRAN's license restrictions, necessary components of the MDSJ layout (which we will use in a minute) are not distributed with \verb@ndtv@.  Instead, the first time the MDSJ layout is called after installing or updating the \verb@ndtv@ package, it is going to ask to download the library.  Lets do that now on a pretend movie to get it out of the way:
...
\subsubsection{Installing Graphviz}
Graphviz is \emph{not} required for \verb@ndtv@ to work.  However it does provide several interesting layouts. Instructions for installing Graphviz can be shown with \verb@?install.graphviz@.


\subsection{Understanding how ndtv works}
...
\subsubsection{Constructing and rendering a dynamicNetwork}

We are going to build a simple \verb@dynamicNetwork@ object ``by hand'' and then visualize its dynamics\footnote{The networkDynamic() command provides utilities for constructing dynamic networks from various data sources, see some the later examples}. Please follow along by running these commands in your R terminal. 
...
Next we add some edges with activity spells.  The edges' connections are determined by the vector of \verb@tail@ and \verb@head@ ids, and the start and ending time for each edge is specified by the \verb@onset@ and \verb@terminus@ vectors.
<<trivial_example2>>=
...
@
Adding active edges to a network has the side effect of converting it to a \verb@dynamicNetwork@. Lets verify it.
...
Once again, we can peek at the edge dynamics by transforming a view of the network into a \verb@data.frame@.
<<trivial_example_verify2>>=
as.data.frame(wheel)
@

When we look at the output \verb@data.frame@, we can see that it did what we asked. For example, edge id 1 connects the ``tail'' vertex id 1 to ``head'' vertex id 2  and has a duration of 10, extending from the ``onset'' of time 1 until the ``terminus'' of time 11. 

It is important to remember that \verb@dynamicNetwork@ objects are also static \verb@network@ objects.  So all of the \verb@network@ functions will still work, they will just ignore the time dimension attached to edges and vertices. For example, if we just \verb@plot@ the network, we see all the edges that ever exist (and realize why the network is named ``wheel''). 
<<trivial_example_plot,fig=TRUE>>=
plot(wheel)
@
...
<<trivial_extract_plot,fig=TRUE>>=
plot(network.extract(wheel,at=1))
@

The \verb@network.extract@ function is one of the many tools provided by the \verb@networkDynamic@ package for storing and manipulating the time information attached to networks without having to work directly with the low-level data structures. The command \verb@help(package='networkDynamic')@ will give a listing of the help pages for all of the functions.   

\smallskip
\noindent\fbox{\parbox{\textwidth}{\textbf{Exercise:} Use the help function to determine the difference between the network.extract() and network.collapse() functions}}
\smallskip

The activity for each edge is stored in an attribute named \verb@activity@ as a matrix of starting and ending times (which we refer to as ``onset'' and ``terminus''). The help page \verb@?activity.attribute@ is a good place to learn more detail about how the \verb@networkDynamic@ package represents dynamics. For many tasks, we would use higher-level methods like \verb@get.edgeIDs.active()@ but, we can access timing information directly using \verb@get.edge.activity@.
<<trival_example_activity>>=
...
\smallskip
\noindent\fbox{\parbox{\textwidth}{\textbf{Question:} Why is this edge labeling function not general enough for some networks? (Hint: do edges always have a single onset and terminus time?)}}
\smallskip
...
\begin{enumerate}
\item Determining appropriate parameters (time range, aggregation rule, etc)
\item Computing layout coordinates for each time slice
\item Rendering a series of plots for each time slice
\item Replaying the cached sequence of plots (or writing to a file on disk)
\end{enumerate}

When we called \verb@render.animation()@ we asked the package to create an animation for \verb@wheel@ but we didn't include any arguments indicating what should be rendered or how, so it had to make some educated guesses or use default values. For example, it assumed that the entire time range of the network should be rendered and that we should use the Kamada-Kawai layout to position the vertices.
    
The process of positioning the vertices was managed by the \verb@compute.animation()@ function which stepped through the \verb@wheel@ network and called a layout function to compute vertex coordinates for each time step. 

Next, \verb@render.animation()@ looped through the network and used \verb@plot.network()@ to render appropriate slice network for each time step.  It calls the \verb@animation@ package function \verb@ani.record()@ to cache the frames of the animation. Finally, \verb@ani.replay()@ quickly redrew the sequence of cached images in the plot window as an animation.

\subsubsection{Controlling the animation processing steps}

For more precise control of the processes, we can call each of the steps in sequence and explicitly set the parameters we want for the rendering and layout algorithms. First we will define a \verb@slice.par@, which is a list of named parameters to specify the time range that we want to compute and render.

<<wheel_detail_slicepar>>==
slice.par=list(start=1, end=12, interval=1, aggregate.dur=1,
              rule='latest')
@

Then we ask it to compute the coordinates for the animation, passing in the \verb@slice.par@ list. The \verb@animation.mode@ argument specifies which algorithm to use.
...
We can see that in addition to the standard vertex attributes of \verb@na@ and \verb@vertex.names@, the network now has two dynamic ``TEA'' attributes for each vertex to describe its position over time. The \verb@slice.par@ argument is also cached as a network attribute so that later on \verb@render.animation()@ will know what range to render.
...
This is essentially what \verb@render.animation()@ does internally.  The standard network plotting arguments are accepted by \verb@render.animation@ (via \verb@...@) and will be passed to \verb@plot.network()@:
...
\item \verb@saveGIF()@ creates an animated GIF (if ImageMagick's  \verb@convert@ is installed)
\item \verb@saveLatex()@ creates an animation embedded in a PDF document
\end{itemize}
Please see \verb@?animation@ and each function's help files for more details and parameters.  We will quickly demonstrate some useful options below.

\subsubsection{Video files}
Since we just rendered the ``wheel'' example movie, it is already cached and we can capture the output of \verb@ani.replay()@ into a movie file. Try out the various output options below.

Save out an animation, providing a non-default filename for the video file:
...
We can increase the video's image quality (and file size) by telling ffmpeg to use a higher bit-rate (less compression)\footnote{This bit-rate setting seems to mostly impacts jpeg output more than the PNG default.}.
<<save_video_bitrate>>==
saveVideo(ani.replay(),video.name="wheel_movie.mp4",
          other.opts="-b 5000k")
@

Because the \verb@ani.record()@ and \verb@ani.replay()@ functions cache each plot image in memory, they are not very speedy and the rendering process will tend to slow to a crawl down as memory fills up when rendering large networks or long movies.  We can avoid this by saving the output of \verb@render.animation@ directly to disk by wrapping it inside the \verb@saveVideo()@ call and setting \verb@render.cache='none'@.
...
\subsubsection{Animated GIF files}
We can also export an animation as an animated GIF image. GIF animations will be very large files, but are very portable for sharing on the web. To render a GIF file, you must have ImageMagick installed. See \verb@?saveGIF@ for more details. 

<<save_GIF,results=hide>>==
saveGIF(render.animation(wheel,vertex.col='blue',
          edge.col='gray',render.cache='none'),
         movie.name="wheel_movie.gif")
@

\subsubsection{Animated PDF/Latex files}
The \verb@animation@ package supports including animations inside PDF documents if you have the appropriate Latex utilities installed. However, the animations will only play inside Adobe Acrobat PDF viewers so it is probably less portable than using GIF or video renders.
...
\smallskip
\noindent\fbox{\parbox{\textwidth}{\textbf{Exercise:} Using the list of options from the help page ?ani.options, locate the option to control the time delay interval of the animation, and use it to render a video where each frame stays on screen for 2 seconds.}}
\smallskip

<<save_video_slow,results=hide>>==
saveVideo(render.animation(wheel,vertex.col='blue',
          edge.col='gray',render.cache='none',
...
\section{Using ndtv Effectively}
\subsection{Animated Layout Algorithms}
First some background about graph layouts.  Producing layouts of dynamic networks is generally a computationally difficult problem. And the definition of what makes a layout ``good'' is often ambiguous or very specific to the domain of the data being visualized. The ndtv package aims for the folowing sometimes conflicting animation goals:

\begin{itemize}
\item Similar layout goals as static layouts (minimize edge crossing, vertex overlap, etc)
\item Changes in network structure should be reflected in changes in the vertex positions in the layout.
\item Layouts should remain as visually stable as possible over time.
...
\subsubsection{Why we avoid Fruchterman-Reingold}
The Fruchterman-Reingold algorithm has been one of the most popular layout algorithms for graph layouts (it is the default for \verb@plot.network@).  For larger networks it can be tuned to run much more quickly than most MDS algorithms.  Unfortunately, its default optimization technique introduces a lot of randomness, so the ``memory'' of previous positions is usually erased each time the layout is run\footnote{Various authors have had useful animation results by modifying FR to explicitly include references to vertices' positions in previous time points. Hopefully we will be able to include such algorithms in future releases of ndtv.}, producing very unstable layouts when used for animations.. 

\subsubsection{Kamada-Kawai adaptation}
The Kamada-Kawai network layout algorithm is often described as a ``force-directed'' or ``spring embedder'' simulation, but it is mathematically equivalent to some forms of MDS\footnote{Kamada-Kawai uses Newton-Raphson optimization instead of SMACOF stress-majorization}. The function \verb@network.layout.animate.kamadakawai@ is essentially a wrapper for \verb@network.layout.kamadakawai@. It computes a symmetric geodesic distance matrix from the input network using \verb@layout.distance@ (replacing infinite values with \verb@default.dist@), and seeds the initial coordinates for each slice with the results of the previous slice in an attempt to find solutions that are as close as possible to the previous positions. It is not as fast as MDSJ, and the layouts it produces are not as smooth. Isolates often move around for no clear reason. But it has the advantage of being written entirely in R, so it doesn't have the pesky external dependencies of MDSJ. For this reason it is the default layout algorithm. 

<<kk_example,results=hide>>=
...
\subsubsection{Graphviz layouts}

The Graphviz \citep{graphviz} external layout library includes a number of excellent algorithms for graph layout, including \verb@neato@, an stress-optimization variant, and \verb@dot@ a hierarchical layout (for trees and DAG networks). As Graphviz is not an R package, you must first install the the software on your computer following the instructions at \verb@?install.graphviz@ before you can use the layouts. The layout uses the \verb@export.dot@ function to write out a temporary file which it then passes to Graphviz.

<<graphviz_example,results=hide>>=
...
\subsubsection{Existing coordinates or customized layouts}
The \verb@network.layout.animate.useAttribute@ layout is useful if you already know exactly where each vertex should be drawn at each time step (based on external data such as latitude and longitude), and you just want \verb@ndtv@ to render out the network. It just needs to know the names of the dynamic TEA attribute holding the x coordinate and the y coordinate for each time step. If no suitable attributes are found, it will just produce an error.

<<useAttr_example,reults=hide,eval=FALSE>>=
compute.animation(short.stergm.sim,animation.mode='useAttribute')
@

It is also possible to write your own layout function and easily plug it in by defining a function with a name like \verb@network.layout.animate.MyLayout@.  See the \verb@ndtv@ package vignette for a circular layout example \verb@browseVignette(package='ndtv')@.

\smallskip
\noindent\fbox{\parbox{\textwidth}{\textbf{Excercise:} Compare the algorithms by watching the video outputs or watch this side-by-side composite version: \url{http://statnet.org/movies/layout_compare.mp4}. What differences are noticeable?}}
\smallskip

<<composite_video_examples,echo=FALSE,eval=FALSE,results=hide>>=
...
More detailed information about each of the layouts and their parameters can be found on the help pages, for example \verb@?network.layout.animate.kamadakawai@.


\subsection{Slicing and aggregating time}
...
\subsubsection{Slicing panel data}

In both the \verb@wheel@ and \verb@short.stergm.sim@ examples, we've been implicitly slicing up time in discrete way, extracting a static network at each unit time step.
...
Following the convention statnet uses for representing ``discrete-time`` simulation dynamics, the edge spells always have integer lengths and extend the full duration of the slice, so it wouldn't actually matter if we used a shorter \verb@aggregate.dur@. Each slice will still intersect with the same set of edges. 
...
\subsubsection{Slicing streaming data}

Slicing up a dynamic network created from discrete panels may appear to be fairly straightforward but it is much less clear how to do it when working with continuous time or ``streaming'' relations where event are recorded as a single time point. How often should we slice?  Should the slices measure the state of the network at a specific instant, or aggregate over a longer time period?  The answer probably depends on what the important features to visualize are in your data-set. The \verb@slice.par@ parameters make it possible to experiment with various aggregation options. In many situations we have even found \citep{sonia} it useful to let slices mostly overlap -- increment each one by a small value to help show fluid changes on a moderate timescale instead of the rapid changes happening on a fine timescale. 

As an example, lets look at the McFarland \citep{mcfarland} data-set of streaming classroom interactions and see what happens when we chop it up in various ways. 
<<mcfarla,fig=TRUE>>=
...
First, we can animate at the fine time scale, viewing the first half-hour of class using instantaneous slices (\verb@aggregate.dur=0@).
<<mcfarla1,results=hide>>=
...
The first movie may give us a good picture of the sequence of conversational turn-taking, but it is hard to see larger structures. If we aggregate over a longer time period of 2.5 minutes we start to see the individual acts form into triads and groups\footnote{\url{http://statnet.csde.washington.edu/movies/ndtv_vignette/cls33_10_16_96v2.mp4}}. 
...
\smallskip
\noindent\fbox{\parbox{\textwidth}{\textbf{Question:} How would measurements of a network's structual properties (degree, etc.) at each time point likely differ in each of the aggregation scenarios? }}
\smallskip
...
timeline(windsurfers,plot.edge.spells=FALSE,vertex.cex=3)
@
...
\smallskip
\noindent\fbox{\parbox{\textwidth}{\textbf{Question:} How would a researcher determine the ``correct'' aggregation period or data collection interval for a dynamic process? When the ``same'' network is sampled repeatedly, how can we distinguish sampling noise from network dynamics?}}
\smallskip

\subsection{Animating network attributes}
...
\subsubsection{Controlling plot properties using dynamic attributes (TEAs)}
If a network has dynamic attributes defined, they can be used to define graphic properties of the network which change over time.  We can activate some attributes on our earlier ``wheel'' example, setting a dynamic attribute for edge widths. The help file listing and explaining dynamic TEA attributes can be displayed with \verb@?dynamic.attributes@.
...
\subsubsection{Controlling plot properties with special functions}

Sometimes it is awkward or inefficient to pre-generate dynamic attribute values.  Why create and attach another attribute for color if it is just a simple transformation of an existing attribute or measure? We can define a function to transform the observed network values into an appropriate plotting attribute. Recall that R allows us to define new functions whenever we want. For example, we could define \verb@myFunction()@  that will accept \verb@myArgument@, transform it, and return the new value.
...
The \verb@render.animation@ function has the ability to accept the \verb@plot.network@ arguments as R functions.  These functions will be evaluated ``on the fly'' at each time point as the network is rendered and can operate on the attributes and properties of the collapsed network. 

For example, if we wanted to use our previously created ``width'`'' attribute to control the color of edges along with their width, we could define a function to extract the value of the ``width'' edge attribute from the momentary slice network and map it to the red component of an RGB color.  We can render the network, setting \verb@edge.col@ equal to this function. 
...
Notice the \verb@slice@ function argument and the use of \verb@slice@ instead of the original name of the network in the body of the function. The arguments of plot control functions must draw from a specific set of named arguments which will be substituted in and evaluated at each time point before plotting.  The set of valid argument names that can be used in special functions is:
\begin{itemize}
  \item \verb@net@ is the original (un-collapsed) network
  \item \verb@slice@ is the static network slice to be rendered, collapsed with the appropriate onset and terminus
...
Notice that in this example we had to modify the start time using the \verb@slice.par@ setting to avoid time 0 because the \verb@betweenness@ function in the \verb@sna@ package will give an error for a network with no edges.  
...
The main plot commands accept functions as well, so it is possible to do fun things like implement a crude zoom effect by setting \verb@xlim@ and \verb@ylim@ parameters to be dependent on the time. 
...
\subsection{Using and aggregating edge weights}
...
By default, the \verb@collapse.network()@ function returns an ordinary static network.  But setting the argument \verb@rm.time.info=FALSE@ will cause it to add some attributes with crude summary information about the activity of the edges and vertices over the time period of aggregation.
...
Notice the \verb@activity.count@ and \verb@activity.duration@ that are now attached to the edges.  

By default the layout algorithms will assume that all edges should have the same ``ideal'' length, any weights or values attached to the edge will be ignored. But if we do have edge values, either from raw data or by aggregating the edges over time, we might want to have the layout attempt to map that information the desired lengths of the edges.  The \verb@weight.attr@ argument makes it possible to pass in the name of a numeric edge attribute to be used in constructing the \verb@layout.dist@ matrix of desired distances between vertices. 

Lets plot the aggregate network with edge weights ignored.  But instead of using the normal \verb@plot.network@ command alone, we are going to extract the coordinates created by \verb@compute.animation@ and feed them into the plot (this is just for constructing the examples, no need to do this for real movies). We will also use the \verb@activity.duration@ attribute to control the drawing width and labels for the edges.

<<edge_weights_ignore,fig=TRUE,results=hide>>=
...
For comparison, we will compute the layout so that the higher-valued edges draw vertices closer together (the default \verb@weight.dist=FALSE@ considers edge weights to be similarities)
<<edge_weights_similarity,fig=TRUE,results=hide>>=
...
Now compute layout with \verb@weight.dist=TRUE@ so weights are treated as distance and higher-valued edges push vertices further apart.
<<edge_weights_distance,fig=TRUE,results=hide>>=
compute.animation(short.stergm.sim,weight.attr='activity.duration',
...
This suggests a solution of attaching a new edge attribute containing the logged duration, and using that as the \verb@weight.attr@
<<edge_weights_log,fig=TRUE,results=hide>>=
...
\subsection{Adjusting spacing of isolates and components}
We've briefly mentioned the \verb@default.dist@ parameter earlier. Most of the layouts use the \verb@layout.dist@ function to compute a matrix of geodesic path distances between vertices. Because there isn't a way to compute a path distance between vertices that have no connections (or between disconnected components) the empty cells of the matrix are filled in with a default value provided by \verb@default.dist@. This value can be tweaked to increase or decrease the spacing between isolates and disconnected components. However, because this works by essentially introducing invisible edges between all of the elements, it can also introduce some distortion in the layout. The default value for \verb@default.dist@ is \verb@sqrt(network.size(net))@, see \verb@?layout.dist@ for more information. 
...
But then the \verb@default.dist@ value was too small to effectively separate the components, resulting in a lot of unnecessary edge crossing. So lets try an intermediate value.
...
<<default_dist_example4,fig=TRUE,results=hide>>==
...
\subsection{A more complete tergm/stergm example}
This is an expanded version of the demo from the beginning of the tutorial. It illustrates how to run a \verb@tergm@ simulation and some additional useful features for working with \verb@tergm@ model output. 

Let say we want to construct a basic but plausible statistical model of a dynamic network defined as a STERGM, and we want to see what one possible realization of the network might look like.  Using statnet's \verb@tergm@ package, we can  estimate the parameters for an edge formation and dissolution process which produces a network similar to the Florentine business network (\verb@?ergm::flobusiness@) given as input.  First we load up the data.
...
Then we ask \verb@stergm@ to fit a model with a random edge probability (``edges'') plus a type of clustering defined as a decreasing marginal return on the number of edge-wise shared partners (``gwesp''). We also include a random dissolution process on the edges to keep the density from simply increasing at each time step.  For more background on the modeling process please see the \verb@tergm@ workshop materials at \url{http://statnet.csde.washington.edu/workshops/SUNBELT/current/tergm/tergm_tutorial.html}.

<<test_model,eval=true>>==
theta.diss <- log(9)
...
After the model has been estimated, we can take a number of sequential draws from it to see how the network might ``evolve'' over time, and output these discrete  networks as a single \verb@dynamicNetwork@ object. 
<<test_sim,eval=true>>==
stergm.sim.1 <- simulate.stergm(stergm.fit.1,
                    nsim=1, time.slices = 25)
@

Now that we have our dataset, it is time to render the movie. We will specify some render parameters using \verb@render.par@.  The \verb@show.stats@ parameter accepts a tergm summary formula to be evaluated, and print the model statistics for each slice on the appropriate frame of the movie. We can specify the formula we used for formation: \verb@~edges+gwesp(0,fixed=T)@
...
Then we ask it to build the animation, passing in some of the standard \verb@plot.network@ graphics arguments to change the color of the edges and show the labels with a smaller size and blue color. As we are fairly familiar with the output by now, we can suppress it by adding a \verb@verbose=FALSE@ argument.
<<render_animation>>==
...
The answer is yes! We will use the example Harry Potter Support Networks of Goele Bossaert and Nadine Meidert \citep{hpdata}. They have coded the peer support ties between 64 characters appearing in the text of each of the well-known fictional J. K. Rowling novels and made the data available for general use in the form of 6 text formatted adjacency matrices and several attribute files.  You can download and unzip the data files from \url{http://www.stats.ox.ac.uk/~snijders/siena/HarryPotterData.html}, or use the R code below to load in directly from the zip file.  The dataset is also included along with a number of other interesting examples in the \verb@networkDynamic@ object in the \verb@networkDynamicData@ \citep{networkDynamicData} package.
...
To construct a \verb@dynamicNetwork@ object, we will arrange them in a list and then convert it with the \verb@dynamicNetwork()@ utility function.
<<format_hp_data>>=
...
As a courtesy to other users (or our future selves) we will add a special \verb@net.obs.period@ attribute to provide some meta data about the temporal model for this dataset.  See \verb@?net.obs.period@ for more information. 
<<net_obs_hp>>=
...
When we render out the movie it is going to look like \emph{Harry Potter and the Philosopher's Stone}, right?
...
We next need to load in the \verb@networkDynamic@ data object, set up the initial state of the simulation, and then use the function we defined above to propagate the rumor. 
...
\smallskip
\noindent\fbox{\parbox{\textwidth}{\textbf{Question:} Why does the infection fail to spread across many of the edges from infected vertices visable in this animation?}}
\smallskip
...
Now lets use the newly-created \verb@transTree()@ function to find the transmission tree, and plot it.
...
Now lets assemble a dynamic network on \verb@windAni@ frame-by-frame applying the coordinates we saved from the previous layouts of the cumulative and transmission tree networks.
...
\subsection{Setting background colors and margins}
You may have noticed that the network plots we have seen so far have fairly wide margins which allow extra room for labels and annotations.  It is possible to adjust the margins, and other generic plot commands, by passing in \verb@par@ arguments to \verb@render.animation@ via \verb@plot.par@.  For example, we can set all of the margins to zero with the \verb@mar@ command and change the background to gray with \verb@bg='gray'@.  See \verb@?par@ for a list of high-level plot parameters suitable for \verb@plot.par@.
<<render_par,results=hide>>=
render.animation(wheel,plot.par=list(bg='gray',mar=c(0,0,0,0)),
                 edge.col='white')
@

\subsection{Tips for working with large networks}
As the \verb@msm.sim@ example probably demonstrated, rendering animations of large networks can be very computationally intensive.  Here are a number of techniques we have found effective for working with networks that take a lot of time or are simply too large to render directly as animations.

\begin{itemize}
\item Test and refine algorithm settings and graphics parameters on a short sequence or single time slice of larger network before running the animation for its  full duration 
\item Render full animations of large networks directly to disk instead previewing in the R plot window.  See the use of the \verb@saveVideo@ and \verb@render.cache='none'@ arguments in example in the Output Formats section. 
\item Extract and visualize a relevant subset of the network.  For example, for a movie illustrating the effects of concurrency we were able to effectively visualize the relationships among the ~800-vertex infected component extracted from a simulation with a population of 10,000: \url{http://statnet.csde.washington.edu/movies/}. Of course, displaying only a sub-network probably introduces some bias into the viewer's perception of the network's overall properties. 
\end{itemize}
...
\begin{itemize}
\item R's built in documentation feature. Each function has a help page, usually with some examples: i.e. \verb@?render.animation@.  The listing of all the documentation pages can be found with \verb@help(package='ndtv')@ or \verb@help(package='networkDynamic')@
...
\section{Limitations}
Before we create unreasonable expectations, we should be clear that you can't just throw any network into \verb@render.animation@ and expect good results.

\begin{itemize}
\item Not for ``big data''. So far we've mostly used it successfully with networks of <1k vertices and several hundred time steps, but that takes a long time to render.
\item Animations of dense or fully-connected networks may not be useful. 
\item The animation techniques work best when a relatively small number of ties are changing between time slices and vertex turnover rates (births and death) are low. 
\item You may need to think carefully about how to aggregate and smooth the dynamics over time in order to reveal the underlying patterns of interest.  This is analogous to choosing bin-widths for histograms in order to reveal the distributional form of the data.
\item Good results may require considering how the weights and measures of your input data can be translated meaningfully to the implicit ``space'' you are embedding your data in.
\end{itemize}

As the \verb@ndtv@ package is still very much under development, we would greatly appreciate your suggestions and feedback: \url{skyebend@skyeome.net} 
...
\bibitem[Skye Bender-deMoll (2014)]{networkDynamicData}
Skye Bender-deMoll (2014). networkDynamicData: dynamic network datasets. R package version 0.1.0.
\url{http://CRAN.R-project.org/package=networkDynamicData}
...
\bibitem[Jenness S, et. al (2014)]{EpiModel}
Jenness S, Goodreau S, Wang L and Morris M (2014). 
\emph{EpiModel: Mathematical Modeling of Infectious Disease}. The Statnet Project
(\url{http://www.statnet.org}). R package version 0.95, \url{CRAN.R-project.org/package=EpiModel}.
