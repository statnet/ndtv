...
\subsection{Authors}
\begin{itemize}
  \item Developed by Skye Bender-deMoll with members of the statnet \url{http://statnet.org} team. \verb@skyebend\@skyeome.net@
...
library(ndtv)
@
Now we need some dynamic network data to look at. A useful way to generate a dynamic network would be to define a STERGM equation giving basic formation and dissolution processes for the edges and then simulate a sequence network states using the \verb@tergm@ package.  We will demonstrate this in a later example, but for now lets just load in an example dataset. The network \verb@short.stergm.sim@ is the output of a toy STERGM model based on Padgett's Florentine Family Business Ties network \citep(padgett).  
<<pre-load_stergm>>==
...
<<print_stergm,fig=TRUE>>=
print(short.stergm.sim)
plot(short.stergm.sim,displaylabels=TRUE)
@
...
Of course, the standard network plot command ignores the the dynamics of edges and vertices, so it actually plots the network aggregated over time --all the edges and vertices that are ever observed to be active.  If we wanted to just see the original starting network we could first extract the network elements active at the beginning and then plot them. 
<<stergm_extract,fig=TRUE>>=
plot(network.extract(short.stergm.sim,at=1),displaylabels=TRUE)
@
...
An animation is not the only way to display a sequence of views of the network.  We could also use the \verb@filmstrip()@ function that will create a ``small multiple'' plot using frames of the animation to construct a visual summary of the network changes as a static plot.

<<filmstrip,fig=TRUE>>=
filmstrip(short.stergm.sim,displaylabels=FALSE)
@

We can view the dynamics as a timeline or where we plot the active spells of edges and vertices over time (but don't show connectivity)
<<timeline,fig=TRUE>>=
timeline(short.stergm.sim)
@

We are experimenting with a form of timeline or phase plot where vertices are positioned vertically by their proximity so that changes in network structure deflect their lines into new positions.
...
\item When vertex turnover rates (births and death) are high, you need to think carefully about how to aggregate and smooth the dynamics over time in order to reveal the underlying patterns of interest.  This is analogous to choosing bin-widths for histograms in order to reveal the distributional form of the data.
\end{itemize}
...
\subsubsection{``Wheel'' work along example}
...
It is important to remember that \verb@dynamicNetwork@ objects are also static \verb@network@ objects.  So all of the \verb@network@ functions will still work, they will just ignore the time dimension attached to edges and vertices.  Now lets view it as a static plot, but labeling edges with their onset and termination times so we can check our work.
...
\subsubsection{What happened automatically}
...
\begin{enumerate}
  ...
  \item \verb@compute.animation()@ then stepped through the \verb@wheel@ network, using \verb@network.collapse()@ to get the appropriate active network, computing coordinates for each time step -- using the previous step's coordinates as starting points -- and storing them as a TEA attribute. (This was the \verb@"Calculating layout for network slice from time 1 to 2"@ ... part.)
...
\subsubsection{Doing it step by step}

For more precise control of the processes, layout algorithms, etc, we can call each of the steps in sequence. Lets work through and examine the output. First we will define a \verb@slice.par@ list of parameters to specify the time range that should have coordinates computed.
...
Then we ask it to compute the coordinates for the animation.
...
We can see that in addition to the standard vertex attributes of \verb@na@ and \verb@vertex.names@, the network now has two dynamic ``TEA'' attributes for each vertex to describe its position over time. Since the coordinates are stored in the network, we could always just collapse the network at the time point and plot it with the appropriate values:
...
\subsection{Demonstrate output formats}
...
\subsection{Comparisons of Layout algorithms}
...
\subsubsection{Why we don't (yet) use Fruchterman-Reingold}
...
The Graphviz \citep{graphviz} external layout library includes a number of excellent algorithms for graph layout, including \verb@neato@, an stress-optimization variant, and \verb@dot@ a hierarchical layout (for trees and DAG networks). 

\subsubsection{Use a TEA attribute or write your own}
...
In both the \verb@wheel@ and \verb@short.stergm.sim@ examples, we've been implicitly slicing up time, extracting a static network at each unit time step.
...
Slicing up a dynamic network created from discrete panels may be fairly straightforward but it is much less clear how to do it when working with continuous time or streaming relations. How often should we slice?  Should the slices measure the state of the network at a specific instant, or aggregate over a longer time period?  The answer probably depends on what the important features to visualize are in your data-set. The \verb@slice.par@ parameters make it possible to experiment with various aggregation options. In many situations we have even found \citep{sonia} it useful to let slices mostly overlap -- increment each one by a small value to help show fluid changes on a moderate timescale instead of the rapid changes happening on a fine timescale. 
...
\subsection{Animating graphic attributes}
...
\subsubsection{Using dynamic attributes (TEAs)}
...
\subsubsection{Functional plot arguments}
...
Lets do the plot again, still ignoring edge weights.  But this time we are going to extract the computed coords and feed them into the plot (this is just for constructing the examples, don't need to do it for real movies).
<<edge_weights_ignore,fig=TRUE>>=
short.stergm.sim%n%'slice.par'<-list(start=0,end=25,interval=25, 
...
\subsection{Adjusting isolates and components separation with layout.distance}
We've briefly mentioned the \verb@default.dist@ parameter which can be tweaked to increase or decrease the spacing between isolates and disconnected components. The default value for \verb@default.dist@ is \verb@sqrt(network.size(net))@, see \verb@?layout.dist@ for more information. 
...
But in the example above, the \verb@default.dist@ value was too small to effectively separate the components, resulting in a lot of unnecessary edge crossing. 
...
\subsection{Transmission trees and assembled layouts}
...
We next need to load in the data with the edge dynamics, and set up the initial state of the simulation, and then use the function we defined above to propagate the rumor. 
...
Expanding on the introductory example. 
Let say we've got a statistical model of a dynamic network defined as a STERGM, and we want to see what it looks like. 
\begin{itemize}
...
We define some parameters in a list named \verb@render.par@ in order to specify how the movie should be rendered. The \verb@show.stats@ parameter accepts a tergm summary formula to be evaluated, to print the model statistics for each slice on the appropriate frame of the movie. 
...
ani.replay()
@
As we are fairly familiar with the output by now, we can suppress it by adding a \verb@verbose=FALSE@ argument.
...
Notice the \verb@stringsAsFactors=FALSE@ argument to \verb@read.table@. This prevents the strings from being converted into factors, which then may unexpectedly appears as integers causing all kinds of headaches. Since that seems to work, lets load in the rest of the files. 
...
\bibitem[Padgett, John F. (1994)]{Padgett} 
