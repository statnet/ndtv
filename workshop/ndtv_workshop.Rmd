---
title: "statnet's temporal network tools: networkDynamic, ndtv and tsna"
...
* Traditional SNA metrics evaluated repeatedly as time series
...
```
The very thin slices (gray vertical lines) (`aggregate.dur=0`) are not intersecting many edge events (purple numbers) at once so the momentary degrees are mostly low.  We can use the ergm's `meandeg` term to compute the mean degree at each of the 50 timesteps
```{r}
