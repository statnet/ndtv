...
Since this isn't a terribly exciting simulation, lets only calculate coordinates for part of the simulated time period by using the \verb@start@ and \verb@end@ parameters of \verb@slice.par@ to specify a time range. We can also ask it to use the MDSJ layout (assuming MDSJ and Java are installed).
...
Finally we render it, giving the names of the dynamic attributes to be used to control the plotting paramters for edge with, vertex size, and vertex color. 
...
\begin{itemize}
  \item \verb@net@ is the original (uncollapsed) network
...
The gost version (which is the default) offers a compromise by linking the observed spells with a faint dotted line, making it possible to trace the trajectories across time without it appearing that there is much more data availible than is actually the case.  Notice also the use of the \verb@start@ and \verb@end@ paramter to zoom in and render only a portion of the time range. 
...
The \verb@render.d3movie@ can save out a network animation an interactive HTML5 SVG to display in a web browser. Animations are generated using a process nearly identical to \verb@render.animation@. However, instead of using R's plotting functions and the animation library, the relevant information is cached and written into a JSON-formatted file, embedded into a web page along with ndtv-d3 player, and displayed in a web browser.  Details and additional examples are included in the ndtv-d3 vignette: \verb@vignette('ndtv-d3 animation examples',package='ndtv')@

\section{Limitations}
\subsection{Size limits}
Like most network algorithms, the layouts tend to scale quite badly. We generally have only had enough patience to generate moves for networks of less than 1000 vertices.  There also seems to be quite a bit of overhead in the \verb@animation@ package, so the generation process seems to slow down considerably for longer duration networks or when slice or render parameters cause lots of slices to be generated. 
