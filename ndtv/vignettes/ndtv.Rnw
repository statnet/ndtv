...
saveVideo(ani.replay(),video.name="stergm.sim.1.mp4", 
                    other.opts="-b 5000k -pix_fmt yuv420p",clean=TRUE)
@     
...
On some installations, the default output from \verb@SaveVideo()@ (really ffmpeg) produces videos in a slightly non-standard .mp4 format that won't play in Windows Media Player or QuickTime.  Users have reported that adding the \verb@other.opts='-pix_fmt yuv420p"@ corrects the problem. 
