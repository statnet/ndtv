...
We can also get an idea of how we are slicing up the network by using the \verb@timeline()@ function to plot the \verb@slice.par@ parameters against the vertex and edge spells. Our very thin slices (gray virtical lines) (\verb@aggregate.dur=0@) are not intersecting may edge events (purple numbers) at once. 
